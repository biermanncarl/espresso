...
class BondedInteraction(ScriptInterfaceHelper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and is_valid_type(args[0], int):
            ...
            # Load the parameters currently set in the ESPResSo core
            self._params = self._get_params_from_es_core()
            print('self._get_params_from_es_core()', 'done')
...
class BondedInteractions(ScriptObjectRegistry):
    ...
    def __init__(self, *args, **kwargs):
        print('BondedInteractions::__init__')
        super().__init__(*args, **kwargs)
    ...
    def __getitem__(self, key):
        print('BondedInteractions::__getitem__')
    ...
    def __setitem__(self, key, value):
        print('BondedInteractions::__setitem__')
        # Validate arguments
        ...
        # Set a dummy shared_ptr (pointing to an empty Variant)
        print(key)
        print(value)
        self.call_method("insert", key=key, object=value)

        # TODO: Remove?
        # Overwrite the empty Variant with the actual bond
